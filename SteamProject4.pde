...
PImage startScreen, endScreen, starts1, starts2, starts3, starts4, win, lose, instructions;
PFont title;
int screenY = 600;
int screenX = 600;
int totalTime;
boolean miniCheck, winning, checked;
float [] scores;
PowerUp p2;
int stage;
...
void draw() {
  ...
  if (stage == 7) {
    ...
    if (box2d.getBodyPixelCoord(b.body).x > 790) {
      ...
      //translate(600,0);
      AICar maxEnemy;
      if (enemies.get(0).pos.x > enemies.get(1).pos.x) maxEnemy = enemies.get(0);
      else maxEnemy = enemies.get(1);
      if (maxEnemy.pos.y > 800 || b.pos.x > maxEnemy.pos.x) {
        winning = true;
      } else {
        winning = false;
      }
  ...
  if (stage == 8) { 
    ...
    if (!checked) {
      ...
      int worstPlace = 0;
      secondsPassed = (currentTime) - (float)timeKeeper;
      ...
      if (secondsPassed < worstScore) scores[worstPlace] = secondsPassed;
      //println(worstPlace);
    }
    timeKeeper = -1;
  ...
  if (stage == 9) {
    ...
    fill(255);
    for (int i = 0; i < scores.length; i++) {
      text(""+(((int)(scores[i])/60000))+":"+((int)((scores[i])/1000)%60)+":"+(int)((scores[i])%1000), width/3+100, 80*i+80);
      //text(""+scores[i], width/3, 80*i+80);
    }
    text("TOP 5 HIGHSCORES", width/3, 500);
    if (keyPressed && key == 'a') {
      stage = 8;
    }
  }
